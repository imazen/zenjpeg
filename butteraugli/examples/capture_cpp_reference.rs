...
fn generate_test_cases() -> Vec<TestCase> {
    ...
    for (seed_idx, &seed) in seeds.iter().enumerate() {
        for &(w, h) in &[(16, 16), (23, 23), (32, 32), (33, 47), (47, 33)] {
            ...
            let name =
                Box::leak(format!("random_seed{}_shift_10_{}x{}", seed_idx, w, h).into_boxed_str());
            cases.push(TestCase::new(
                name,
                w,
                h,
            ...
            // Random with noise
            let name =
                Box::leak(format!("random_seed{}_noise_20_{}x{}", seed_idx, w, h).into_boxed_str());
...
fn main() {
    ...
    eprintln!(
        "Capturing C++ butteraugli reference data for {} test cases...",
        cases.len()
    );
    eprintln!();
    ...
    for case in &cases {
        ...
        match cpp_butteraugli(&img_a, &img_b, case.width, case.height, intensity_target) {
            Some((score, diffmap)) => {
                ...
                results.push((case.name.to_string(), case.width, case.height, score, stats));
                success_count += 1;
                eprintln!(
                    "  ✓ {} ({}x{}): score={:.6}",
    ...
    println!(
        "pub const REFERENCE_INTENSITY_TARGET: f32 = {:.1};",
        intensity_target
    );
    println!();
