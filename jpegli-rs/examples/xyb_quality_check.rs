...
fn main() {
    ...
    if let Some(cjpegli) = jpegli::test_utils::find_cjpegli() {
        ...
        let rust_decoded_raw = Decoder::new().apply_icc(false).decode(&rust_xyb).unwrap();
        let cpp_decoded_raw = Decoder::new().apply_icc(false).decode(&cpp_xyb).unwrap();
        ...
        let rust_decoded = Decoder::new().apply_icc(false).decode(&rust_xyb).unwrap();
        let cpp_decoded = Decoder::new().apply_icc(false).decode(&cpp_xyb).unwrap();
