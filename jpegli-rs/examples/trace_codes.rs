...
fn main() {
    ...
    for s in [0x60] {
        for _ in 0..3 { counter.count(s); }
    }
    
    // From refinement scans (masked symbols)
    for s in [0x00, 0x01, 0x11, 0x21, 0x30, 0x31, 0x40, 0x51, 0x61, 0x81, 0x91, 0xa1, 0xd1, 0xf0, 0xf1] {
        for _ in 0..5 { counter.count(s); }
    }
    for s in [0x00, 0x20, 0x30, 0x40, 0xf1] {
        for _ in 0..3 { counter.count(s); }
    }
    for s in [0x00, 0x01, 0x10, 0x11, 0x21, 0x31, 0x41, 0x51, 0x61, 0x81, 0x91, 0xa1, 0xc1, 0xf0, 0xf1] {
        for _ in 0..10 { counter.count(s); }
    }
    for s in [0x00, 0x01, 0x11, 0x20, 0x21, 0x31, 0x41, 0x51, 0x61, 0x81, 0x91, 0xa1, 0xb1, 0xc1, 0xd1, 0xe1, 0xf0, 0xf1] {
        for _ in 0..5 { counter.count(s); }
    }
