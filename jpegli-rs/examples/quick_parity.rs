...
fn main() {
    ...
    let strip = Encoder::new()
        ...
        .subsampling(Subsampling::S420)
        .jpegli_quality(Quality::from_quality(90.0))
        .encode_strip_based(&data).unwrap();
    ...
    println!("Full SOS at byte {}, Strip SOS at byte {}", full_sos, strip_sos);
    ...
    println!("Full scan bytes:  {:02x?}", &full[full_scan_start..full_scan_start+50]);
    println!("Strip scan bytes: {:02x?}", &strip[strip_scan_start..strip_scan_start+50]);
}
