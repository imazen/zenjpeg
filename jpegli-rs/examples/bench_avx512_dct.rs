...
fn main() {
    #[cfg(all(feature = "archmage-simd", target_arch = "x86_64"))]
    {
        ...
        if let Some(token) = Desktop64::summon() {
            ...
            println!("AVX2 single-block: {:.2}M blocks/sec ({:.2}ms for {}K blocks)", 
                     blocks_per_sec / 1e6, elapsed.as_secs_f64() * 1000.0 / ITERS as f64, BLOCKS * 2 / 1000);
        }
        ...
        if let Some(token) = Avx512fToken::try_new() {
            ...
            for i in (0..BLOCKS * 2).step_by(2) {
                ...
                mage_forward_dct_8x8_dual(token, &input[i], &input[i + 1], &mut left[i], &mut right[0]);
            }
            ...
            let elapsed = start.elapsed();
            let blocks_per_sec = (BLOCKS * 2 * ITERS) as f64 / elapsed.as_secs_f64();
            println!("AVX-512 dual-block: {:.2}M blocks/sec ({:.2}ms for {}K blocks)", 
                     blocks_per_sec / 1e6, elapsed.as_secs_f64() * 1000.0 / ITERS as f64, BLOCKS * 2 / 1000);
        } else {
