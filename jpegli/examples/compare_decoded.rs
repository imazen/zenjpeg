...
fn main() {
    ...
    for (png_path, name) in test_images {
        if let Some(result) = compare_image(png_path, name) {
            ...
            println!("{:<20} {:>7.1}% {:>7.1}% {:>7.1}% {:>7.1}% {:>8} {:>6.1}",
                     name,
...
fn compare_image(png_path: &str, name: &str) -> Option<CompareResult> {
    ...
    if cpp_decoded.len() != rust_decoded.len() { return None; }
    ...
    Some(CompareResult { total_pixels, exact_match, off_by_1, off_by_2, off_by_3_plus, max_diff, psnr })
}
