...
fn main() {
    ...
    let y_plane: Vec<f32> = rgb.chunks(3)
        .map(|p| {
    ...
    // Distribution
    println!("\nDistribution:");
    let buckets = [0.0, 0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.14, 0.16, 0.20, 0.30];
    for i in 0..buckets.len()-1 {
        let count = values.iter().filter(|&&v| v >= buckets[i] && v < buckets[i+1]).count();
        let pct = count as f32 / values.len() as f32 * 100.0;
        println!("  [{:.2}, {:.2}): {} ({:.1}%)", buckets[i], buckets[i+1], count, pct);
    }
}
