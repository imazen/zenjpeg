...
fn main() {
    ...
    println!("H-gradient DCT (first row): {:?}",
             gradient_dct[..8].iter().map(|x| format!("{:.2}", x)).collect::<Vec<_>>());
