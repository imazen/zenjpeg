...
fn main() {
    ...
    for i in 1..6 {
        println!("  AC[{}]: Rust={:.4}, C++={:.4}, diff={:.6}",
                 i, dct_rust[i], dct_cpp[i], (dct_rust[i] - dct_cpp[i]).abs());
    }
    ...
    println!("DC: Rust={:.2}, C++ raw={:.2}, C++ adjusted={:.2}",
             dct_grad_rust[0], dct_grad_cpp[0], dct_grad_cpp[0] - 128.0);
    println!("Rust DC - C++ adjusted DC: {:.2}", dct_grad_rust[0] - (dct_grad_cpp[0] - 128.0));
