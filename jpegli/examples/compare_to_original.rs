...
fn main() {
    let test_images = [
        ("/home/lilith/work/jpegli/testdata/jxl/flower/flower_small.rgb.png", "flower"),
    ...
    for (png_path, name) in test_images {
        if let Some(result) = compare_to_original(png_path, name) {
            println!("{:<20} {:>12.2} {:>12.2} {:>12} {:>12}",
                     name, result.cpp_psnr, result.rust_psnr, 
                     result.cpp_max_err, result.rust_max_err);
        }
    }
    ...
    if let Some(result) = compare_to_original(
        ...
        println!("C++  vs Original: PSNR={:.2} dB, MSE={:.2}, MaxErr={}", 
                 result.cpp_psnr, result.cpp_mse, result.cpp_max_err);
        println!("Rust vs Original: PSNR={:.2} dB, MSE={:.2}, MaxErr={}", 
                 result.rust_psnr, result.rust_mse, result.rust_max_err);
        println!("C++  vs Rust:     PSNR={:.2} dB, MSE={:.2}, MaxErr={}", 
                 result.cross_psnr, result.cross_mse, result.cross_max_err);
        
        println!("\nC++ better on {} pixels ({:.1}%)", 
                 result.cpp_better, 100.0 * result.cpp_better as f64 / result.total as f64);
        println!("Rust better on {} pixels ({:.1}%)", 
                 result.rust_better, 100.0 * result.rust_better as f64 / result.total as f64);
        println!("Same distance: {} pixels ({:.1}%)", 
                 result.same, 100.0 * result.same as f64 / result.total as f64);
    }
}
...
fn compare_to_original(png_path: &str, name: &str) -> Option<CompareResult> {
    ...
    Command::new("/home/lilith/work/jpegli/build/tools/cjpegli")
        .args(["--noadaptive_quantization", "--chroma_subsampling=444", "-p", "0",
               "--fixed_code", &ppm_path, &cpp_jpg_path, "-q", "90"])
        .output().ok()?;
    ...
    let cpp_psnr = if cpp_mse > 0.0 { 10.0 * (255.0_f64.powi(2) / cpp_mse).log10() } else { 100.0 };
    let rust_psnr = if rust_mse > 0.0 { 10.0 * (255.0_f64.powi(2) / rust_mse).log10() } else { 100.0 };
    let cross_psnr = if cross_mse > 0.0 { 10.0 * (255.0_f64.powi(2) / cross_mse).log10() } else { 100.0 };
    
    Some(CompareResult {
        cpp_psnr, rust_psnr, cpp_mse, rust_mse, cpp_max_err, rust_max_err,
        cross_psnr, cross_mse, cross_max_err, cpp_better, rust_better, same, total,
    })
}
