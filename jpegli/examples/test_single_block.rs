...
fn main() {
    ...
    println!("First row: {:?}", &y_block[0..8].iter().map(|x| format!("{:.1}", x)).collect::<Vec<_>>());
    ...
    println!("First row: {:?}", &y_shifted[0..8].iter().map(|x| format!("{:.1}", x)).collect::<Vec<_>>());
    ...
    println!("First row: {:?}", &y_dct[0..8].iter().map(|x| format!("{:.2}", x)).collect::<Vec<_>>());
    ...
    println!("Non-zero count: {}", quantized.iter().filter(|&&x| x != 0).count());
