...
fn compute_dssim(original: &[u8], distorted: &[u8], width: usize, height: usize) -> f64 {
    let attr = Dssim::new();
    let orig_rgba: Vec<RGBA8> = original.chunks(3).map(|c| RGBA8::new(c[0], c[1], c[2], 255)).collect();
    let dist_rgba: Vec<RGBA8> = distorted.chunks(3).map(|c| RGBA8::new(c[0], c[1], c[2], 255)).collect();
...
fn main() {
    ...
    let output_path = std::env::args().nth(1).unwrap_or_else(|| "reports/low_q_report.html".to_string());
    ...
    for &q in &qualities {
        ...
        Command::new("/home/lilith/work/jpegli/build/tools/cjpegli")
            .args(["--chroma_subsampling=444", "-p", "0", "--fixed_code", 
                   "/tmp/test.ppm", &cpp_path, "-q", &q.to_string()])
            .output().unwrap();
...
"#);
    ...
    html.push_str(r#"<div class="summary"><h2>Key Findings</h2><ul>
...
"#);
    ...
    // Chart
    html.push_str(r#"<h2>Rate-Distortion Curve</h2><div id="rd-chart" class="chart"></div><script>
"#);
    
    let configs = [("YCbCr", "C++", "#1976d2"), ("YCbCr", "Rust", "#42a5f5"), 
                   ("XYB", "C++", "#c62828"), ("XYB", "Rust", "#ef5350")];
    let mut trace_names = Vec::new();
    
    for (i, (mode, encoder, color)) in configs.iter().enumerate() {
        let filtered: Vec<&TestResult> = results.iter()
        ...
        let labels: Vec<String> = filtered.iter().map(|r| format!("'Q{}'", r.quality)).collect();
        
        html.push_str(&format!(
            "var t{}={{x:[{}],y:[{}],mode:'lines+markers',name:'{} {}',text:[{}],line:{{color:'{}'}}}};\n",
            i, bpp.join(","), dssim.join(","), mode, encoder, labels.join(","), color
        ));
        trace_names.push(format!("t{}", i));
    }
    
    html.push_str(&format!(
        "Plotly.newPlot('rd-chart',[{}],{{xaxis:{{title:'bpp'}},yaxis:{{title:'DSSIM (log)',type:'log'}}}});</script>\n",
        trace_names.join(",")
    ));
    ...
    for mode in ["YCbCr", "XYB"] {
        ...
        html.push_str("<tr><th>Q</th><th>C++ Size</th><th>Rust Size</th><th>Δ</th><th>C++ DSSIM</th><th>Rust DSSIM</th></tr>\n");
        
        for &q in &qualities {
            let cpp = results.iter().find(|r| r.quality == q && r.mode == mode && r.encoder == "C++");
            let rust = results.iter().find(|r| r.quality == q && r.mode == mode && r.encoder == "Rust");
            ...
            if let (Some(c), Some(r)) = (cpp, rust) {
                ...
                html.push_str(&format!(
                    "<tr><td>{}</td><td>{}</td><td>{}</td><td class='{}'>{:+.1}%</td><td>{:.6}</td><td>{:.6}</td></tr>\n",
                    q, c.size, r.size, class, delta, c.dssim, r.dssim
                ));
            }
        }
        html.push_str("</table>\n");
    ...
    html.push_str(&format!("<p><em>Generated: {}</em></p></body></html>", 
    ...
    for mode in ["YCbCr", "XYB"] {
        ...
        for &q in &[30, 60, 90] {
            ...
            if let (Some(c), Some(r)) = (cpp, rust) {
                let delta = (r.size as f64 - c.size as f64) / c.size as f64 * 100.0;
                println!("  Q{}: C++ {} Rust {} ({:+.1}%), DSSIM {:.6} vs {:.6}", 
