...
//!
//! Currently implements BT.601 full-range (JFIF) RGB→YCbCr for 4:4:4 and 4:2:0
//! subsampling at 8-bit depth. Matches the `yuv` crate's Professional mode
//! output (max ±1 level) while being slightly faster on x86-64 AVX2.
//!
//! # Dispatch tiers
//!
//! - **x86-64 AVX2** — `#[arcane]` pmaddwd kernel (32 pixels/iter for 4:4:4,
//!   2×32 fused Y+chroma for 4:2:0). Entry point selected at runtime via
//!   `X64V3Token::summon()`.
