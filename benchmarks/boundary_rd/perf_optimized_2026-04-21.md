...
## Raw artifacts (regenerated by the reproduction commands above)

These intermediate CSVs and callgrind dumps (50 × 5-iter perf CSVs per
optimization stage, plus per-stage callgrind_annotate outputs) were
produced during the optimization pass but are not committed — they
are reproducible and would bloat the repo. The headline numbers
