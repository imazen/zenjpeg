...
### Inter-Stage Batching (2026-01-30)

**Attempted:** Batch strips through pipeline stages (all color→all DCT→all quant) instead
of the fused per-strip order (color→AQ→DCT→quant per strip). Hypothesis: batched processing
keeps icache warm per stage and gives prefetcher more predictable patterns.

**Cachegrind baseline analysis (4096×2160, 4:2:0, Q85):**

| Stage | D1 Read Miss Rate | Notes |
|-------|-------------------|-------|
| Color Convert | 3.70% | Data from input buffer |
| AQ | 7.45% | Inherent streaming pattern across full width |
| Deringing | 0.04% | Perfect producer-consumer locality in fused path |
| DCT | 2.98% | Strip buffers fit in L2 |
| Quantize | 3.91% | Sequential pending block reads |

Working set: 896 KB (88% of typical 1 MB L2). Strip buffers already fit in L2.

**Wall-clock results (median of 10-20 runs):**

| Image | Size | Fused ms | Best Batch | Diff | Worst Batch | Diff |
|-------|------|----------|------------|------|-------------|------|
| Synthetic | 512×512 | 1.24 | batch=8: 1.23 | -0.6% | batch=1: 1.52 | +22.6% |
| Synthetic | 2048×2048 | 20.75 | batch=2: 21.01 | +1.3% | batch=4: 23.86 | +15.0% |
| Synthetic | 4096×2160 | 43.78 | batch=2: 42.87 | -2.1% | batch=16: 50.26 | +14.8% |
| Photo (CLIC) | 1507×2048 | 16.60 | batch=8: 15.98 | -3.7% | batch=1: 17.34 | +4.5% |
| Photo (CLIC) | 4096×2160 | 44.47 | batch=8: 45.85 | +3.1% | batch=16: 59.35 | +33.5% |

**Why batching doesn't help (and often hurts):**
1. **Strip buffers already fit in L2** — no inter-stage eviction to recover from
2. **Deringing has 0.04% D1 miss rate** in fused mode (perfect producer-consumer locality).
   Batching breaks this by forcing a save/restore round-trip.
3. **AQ's 7.45% miss rate is inherent** — streaming access across 4096×8 rows, not stage transitions
4. **Extra memcpy** — batching requires saving/restoring intermediate strip data (~768 KB per strip),
   which adds memory bandwidth overhead
5. **Large batches cause L3 pressure** — batch=16 at 4K stores ~12 MB of intermediates, polluting L3

**Conclusion:** The fused per-strip architecture is correct. C++ jpegli also uses this approach.
Any differences within ±3% are measurement noise. Large batches consistently hurt (10-34% slower)
due to working set exceeding cache and extra memcpy. The experiment confirms that inter-stage
cache thrashing is not a performance bottleneck.

**Files:** `zenjpeg/src/encode/strip/mod.rs` (`process_strips_batched`, `BatchReuse`),
`zenjpeg/examples/bench_batched_pipeline.rs`
