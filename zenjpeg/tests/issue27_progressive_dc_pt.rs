...
const CRASH_JPEG: &[u8] = &[
    ...
    0x18, 0x12, 0x14, 0x15, 0x14, 0xff, 0xc2, 0x00, 0x0b, 0x08, 0x00, 0x08,
    0x00, 0x08, 0x01, 0x01, 0x11, 0x00, 0xff, 0xc4, 0x00, 0x14, 0x00, 0x01,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0xff, 0xda, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00,
    0x00, 0x01, 0x54, 0xff, 0x00, 0xff, 0xd9,
];
...
fn test_issue27_progressive_dc_point_transform_al1() {
    ...
    let result = decoder.decode(CRASH_JPEG, Unstoppable).expect("decode failed");
    ...
    for (i, &px) in pixels.iter().enumerate() {
        let diff = if px > expected { px - expected } else { expected - px };
        if diff > max_diff {
            max_diff = diff;
            eprintln!("pixel[{}] = {}, expected {}, diff {}", i, px, expected, diff);
        }
    }
    eprintln!("max_diff = {}", max_diff);

    // All 64 pixels should be 255
    assert!(max_diff <= 1, "max_diff={max_diff}, expected all pixels to be 255 (or 254 due to rounding)");
}
