...
fn verify_arithmetic_coefficients_match_huffman_transcode() {
    ...
    let ari_coeffs = decoder
        .decode_coefficients(&ari_data, Unstoppable)
        .expect("failed to decode arithmetic");

    // Decode Huffman-transcoded version (jpegtran -copy none)
    let huff_data =
        std::fs::read("/tmp/testimgari_huffman.jpg").expect("failed to read huffman file");
    let huff_coeffs = decoder
        .decode_coefficients(&huff_data, Unstoppable)
        .expect("failed to decode huffman");

    // Compare coefficient counts
    assert_eq!(
        ari_coeffs.components.len(),
        huff_coeffs.components.len(),
        "component count mismatch"
    );

    // Compare each component
    for (ci, (ari_comp, huff_comp)) in ari_coeffs
        .components
        .iter()
        .zip(huff_coeffs.components.iter())
        .enumerate()
    {
        assert_eq!(
            ari_comp.coeffs.len(),
            huff_comp.coeffs.len(),
            "component {} coefficient count mismatch",
            ci
        );
        ...
        for (i, (&a, &h)) in ari_comp
            .coeffs
            .iter()
            .zip(huff_comp.coeffs.iter())
            .enumerate()
        {
            ...
            if diff != 0 {
                ...
                if diff_count <= 10 {
                    ...
                    let pos = i % 64;
                    println!(
                        "Component {}, Block {}, Position {}: ari={}, huff={}, diff={}",
        ...
        println!(
            "Component {}: {} total diffs, max diff = {}",
