...
/// Published crates in this workspace (`jpegli-internals-sys` and
/// `zenjpeg-bench-utils` are internal `publish = false`; `zjpeg` and
/// `zjr-calibrate` are bin-only).
///
/// zenjpeg is snapshotted with default features only: `--all-features` would
/// enable `layout` (dep:zenresize) and `__picker-research` (unpublished
/// sibling path-dep zenpredict), both of which CI's "Strip non-CI workspace
/// members and deps" step empties/removes — an all-features snapshot
/// generated locally could never match the CI `check` regeneration.
const CRATES: &[CrateCfg] = &[
    ("zenjpeg", &[("default features", &[])]),
    (
        "zenyuv",
        &[
            ("default features", &[]),
    ...
    for (package, configs) in CRATES {
        ...
        doc.push_str(&format!(
            "# {package} public API surface\n\
             # Generated by zenjpeg/tests/public_api_doc.rs via `cargo public-api --simplified`\n\
