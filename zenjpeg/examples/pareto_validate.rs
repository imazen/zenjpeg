...
fn main() {
    ...
    let images: Vec<_> = if corpus.exists() {
        std::fs::read_dir(corpus).expect("read").filter_map(|e| e.ok())
        ...
        std::fs::read_dir(fallback).expect("read").filter_map(|e| e.ok())
            .filter(|e| e.path().extension().map(|x| x == "png" || x == "jpg").unwrap_or(false))
    ...
    for (name, coupling, quality) in &configs {
        ...
        if *coupling == 0.0 {
            ...
            let pareto = if size_delta < 0.0 && ba_delta < 0.0 { " ★" } else { "" };
            println!("{:>30} {:>10.0} {:>10.3} {:>+10.1}% {:>+10.1}%{}",
    ...
    for base_q in [75.0, 80.0, 85.0, 90.0, 95.0] {
        ...
        let mut bl_sz = 0usize; let mut bl_ba = 0.0;
        let mut hy_sz = 0usize; let mut hy_ba = 0.0;
        let mut cnt = 0;
        
        for img in &loaded {
            if let (Some(bl), Some(hy)) = (encode_with_config(&baseline_cfg, img),
                                            encode_with_config(&hybrid_cfg, img)) {
                if let (Some(bl_b), Some(hy_b)) = (compute_butteraugli(img, &bl),
                                                    compute_butteraugli(img, &hy)) {
                    bl_sz += bl.len(); bl_ba += bl_b;
        ...
        println!("{:>6.0} {:>10.0} {:>10.3} {:>10.0} {:>10.3} {:>+10.1}% {:>+10.1}%{}",
