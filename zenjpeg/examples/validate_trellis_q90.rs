    ...
    for img in &loaded {
        if let (Some(jj), Some(tj)) = (encode(&jpegli_cfg, img), encode(&trellis_cfg, img)) {
            if let (Some(jb), Some(tb)) = (ba(img, &jj), ba(img, &tj)) {
                ...
                println!("{:>30} {:>10} {:>10} {:>+10.1}% {:>+10.1}% {:>8}",
                         name, jj.len(), tj.len(), sz_d, ba_d, winner);
            }
        }
    }
    
    println!("\n=== Summary ===\n");
    println!("Q88-95 Pareto analysis: {} wins, {} losses", pareto_wins, pareto_losses);
    println!("Q90 per-image: Trellis wins {}, Jpegli wins {}, Ties {}", 
