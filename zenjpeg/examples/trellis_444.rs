...
fn main() {
    ...
    let images: Vec<_> = if Path::new(base_dir).exists() {
        std::fs::read_dir(base_dir).unwrap().filter_map(|e| e.ok())
            ...
            .map(|e| e.path().join("original.png")).collect()
    } else {
    ...
    for img in &loaded {
        if let Some(j) = encode(&cfg_420, img) {
            if let Some(b) = ba(img, &j) { sz += j.len(); ba_v += b; n += 1; }
        }
    }
    let (base_sz, base_ba) = (sz as f64 / n as f64, ba_v / n as f64);
    println!("{:>15} {:>12.0} {:>8.3} {:>12} {:>8}", "Trellis 4:2:0", base_sz, base_ba, "-", "-");
    ...
    println!("{:>15} {:>12.0} {:>8.3} {:>+12.1}% {:>+8.1}%", 
             "Trellis 4:4:4", sz as f64 / n as f64, ba_v / n as f64, sz_d, ba_d);
}
