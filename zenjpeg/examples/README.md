...
This README indexes the most useful, durable tools — a curated subset of the
full `examples/` directory (130+ files, many of them one-off investigation
harnesses kept for reproducibility). `ls examples/` for the complete list;
names follow `<category>_<specific>.rs`. Feature-gated examples declare
`required-features` in `Cargo.toml`, so a wrong invocation fails with the
...
| `low_q_report` | HTML quality report (Rust vs C++) | `cargo run --release --example low_q_report --features __test-utils` |
...
```bash
cargo run --release --example low_q_report --features __test-utils
...
```bash
cargo run --release --example compare_xyb_constants --features "__test-utils,__ffi-tests"
...
# The historical progressive-XYB decode bug is fixed;
# tests/bundled/progressive_xyb_decode.rs remains as the regression guard.
```
...
1. ~~Progressive XYB decode bug~~ **Fixed.** zenjpeg decodes progressive XYB
   JPEGs from C++ cjpegli (non-standard component IDs R=82, G=71, B=66).
   `tests/bundled/progressive_xyb_decode.rs` is the regression guard.

2. **XYB size gap vs C++**: see the current numbers and status in the
   repo-root `CLAUDE.md` (XYB section) rather than this file — they change
   as tuning lands.

## Adding New Examples

When adding debugging tools:
1. Use the `zenjpeg-bench-utils` crate for synthetic images and metrics
