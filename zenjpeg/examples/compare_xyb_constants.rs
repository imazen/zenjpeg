...
use zenjpeg::color::xyb::{SCALED_XYB_OFFSET, SCALED_XYB_SCALE};
use zenjpeg::foundation::consts::{XYB_OPSIN_ABSORBANCE_BIAS, XYB_OPSIN_ABSORBANCE_MATRIX};
...
fn main() {
    ...
    for (r, g, b, name) in &test_colors {
        // Rust conversion
        let (rust_x, rust_y, rust_b) = zenjpeg::color::xyb::srgb_to_scaled_xyb(*r, *g, *b);
    ...
    for i in 0..width * height {
        let (x, y, b) =
            zenjpeg::color::xyb::srgb_to_scaled_xyb(rgb[i * 3], rgb[i * 3 + 1], rgb[i * 3 + 2]);
        rust_xyb[i * 3] = x;
        rust_xyb[i * 3 + 1] = y;
        rust_xyb[i * 3 + 2] = b;
    }
