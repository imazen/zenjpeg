...
use std::path::Path;
use zenjpeg::encode::{ChromaSubsampling, EncoderConfig, OptimizationPreset, PixelLayout, ScanMode};
...
fn test_curve(name: &str, configs: Vec<(f32, EncoderConfig)>, images: &[ImageData]) {
    ...
    for (q, cfg) in configs {
        ...
        println!("{:>6.0} {:>10.0} {:>8.3}", q, sz as f64 / n as f64, b / n as f64);
    }
}
...
fn main() {
    ...
    test_curve("1. JpegliProgressive (baseline)", 
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::JpegliProgressive))).collect(),
        &loaded);
    
    // 2. Trellis (HybridProgressive without coupling)
    test_curve("2. Trellis (HybridProgressive, no coupling)",
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::HybridProgressive))).collect(),
        &loaded);
    
    // 3. Scan optimization (progressive search)
    test_curve("3. Jpegli + Scan Optimization",
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::JpegliProgressive)
            .optimize_scans(true))).collect(),
        &loaded);
    
    // 4. Mozjpeg tables (Robidoux) instead of Jpegli tables
    test_curve("4. MozjpegProgressive (different tables, has trellis)",
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::MozjpegProgressive))).collect(),
        &loaded);
    
    // 5. 4:4:4 subsampling
    test_curve("5. Jpegli 4:4:4 (no chroma subsampling)",
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::None)
            .optimization(OptimizationPreset::JpegliProgressive))).collect(),
        &loaded);
    
    // 6. Deringing
    test_curve("6. Jpegli + Deringing",
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::JpegliProgressive)
            .deringing(true))).collect(),
        &loaded);
    
    // 7. Combined: Trellis + Scan Optimization
    test_curve("7. Trellis + Scan Optimization",
        qs.iter().map(|&q| (q, EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
    ...
    let configs: Vec<(&str, EncoderConfig)> = vec![
        ("JpegliProgressive", EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::JpegliProgressive)),
        ("+ Trellis", EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::HybridProgressive)),
        ("+ Scan Opt", EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::JpegliProgressive)
            .optimize_scans(true)),
        ("+ Deringing", EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::JpegliProgressive)
            .deringing(true)),
        ("MozjpegProg", EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::MozjpegProgressive)),
        ("Trellis+ScanOpt", EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::HybridProgressive)
            .optimize_scans(true)),
    ];
    ...
    for img in &loaded {
        if let Some(j) = encode(base_cfg, img) {
            if let Some(v) = ba(img, &j) { bsz += j.len(); bba += v; bn += 1; }
        }
    }
    let (base_sz, base_ba) = (bsz as f64 / bn as f64, bba / bn as f64);
    
    println!("{:>20} {:>10} {:>8} {:>10} {:>10}", "Config", "Size", "BA", "ΔSize%", "ΔBA%");
    println!("{}", "-".repeat(62));
    ...
    for (name, cfg) in &configs {
        ...
        let marker = if sd < -1.0 && bd < 1.0 { " ★" } else if sd.abs() < 0.5 && bd.abs() < 0.5 { " (same)" } else { "" };
        println!("{:>20} {:>10.0} {:>8.3} {:>+10.1}% {:>+10.1}%{}", name, s, ba_v, sd, bd, marker);
    }
