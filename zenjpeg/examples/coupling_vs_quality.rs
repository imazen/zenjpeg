...
fn encode(config: &EncoderConfig, img: &ImageData) -> Option<Vec<u8>> {
    let mut encoder = config.encode_from_bytes(img.width as u32, img.height as u32, PixelLayout::Rgb8Srgb).ok()?;
...
fn main() {
    ...
    for q in [80.0, 82.0, 84.0, 85.0, 86.0, 88.0, 90.0] {
        ...
        for img in &loaded {
            if let Some(j) = encode(&cfg, img) {
                if let Some(b) = butteraugli(img, &j) { sz += j.len(); ba += b; n += 1; }
            }
        }
    ...
    for q in [82.0, 83.0, 84.0, 85.0, 86.0, 87.0, 88.0] {
        let hybrid = HybridConfig { enabled: true, aq_lambda_scale: 5.0, ..Default::default() };
        let cfg = EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
    ...
    for q in [85.0, 86.0, 87.0, 88.0, 89.0, 90.0] {
        let hybrid = HybridConfig { enabled: true, aq_lambda_scale: -4.0, ..Default::default() };
        let cfg = EncoderConfig::ycbcr(q, ChromaSubsampling::Quarter)
            .optimization(OptimizationPreset::HybridProgressive)
            .hybrid_config(hybrid);
        let (mut sz, mut ba, mut n) = (0usize, 0.0, 0);
    ...
    let hybrid_plus: Vec<_> = points.iter().filter(|p| p.2.starts_with("Hybrid+5")).collect();
    let closest_plus = hybrid_plus.iter().min_by_key(|p| ((p.0 - jp85.0).abs() * 100.0) as i64).unwrap();
    
    let ba_diff = (closest_plus.1 - jp85.1) / jp85.1 * 100.0;
    let winner = if ba_diff < -1.0 { "Hybrid+5" } else if ba_diff > 1.0 { "Jpegli" } else { "~same" };
    println!("{:>20} {:>10.0} {:>8.3}  vs  {:>20} {:>10.0} {:>8.3}  {:>10} ({:+.1}% BA)",
    ...
    let hybrid_minus: Vec<_> = points.iter().filter(|p| p.2.starts_with("Hybrid-4")).collect();
    let closest_minus = hybrid_minus.iter().min_by_key(|p| ((p.0 - jp85.0).abs() * 100.0) as i64).unwrap();
    
    let ba_diff = (closest_minus.1 - jp85.1) / jp85.1 * 100.0;
    let winner = if ba_diff < -1.0 { "Hybrid-4" } else if ba_diff > 1.0 { "Jpegli" } else { "~same" };
    println!("{:>20} {:>10.0} {:>10.3}  vs  {:>20} {:>10.0} {:>8.3}  {:>10} ({:+.1}% BA)",
