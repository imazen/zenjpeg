...
fn compute_butteraugli(img: &ImageData, jpeg: &[u8]) -> Option<f64> {
    ...
    Some(QualityMetrics::butteraugli(orig_rgb.as_ref(), decoded.as_ref()))
}
...
fn main() {
    ...
    let images: Vec<_> = if corpus.exists() {
        std::fs::read_dir(corpus).unwrap().filter_map(|e| e.ok())
        ...
        std::fs::read_dir(fallback).unwrap().filter_map(|e| e.ok())
    ...
    for (name, coupling, quality) in &test_configs {
        ...
        let pareto = if sz_d < -0.5 && ba_d < -0.5 { " ★" } else { "" };
        
        println!("{:>35} {:>10.0} {:>10.3} {:>+10.1}% {:>+10.1}%{}",
    ...
    for q in [75.0, 80.0, 85.0, 90.0, 95.0] {
        ...
        let mut jp_sz = 0usize; let mut jp_ba = 0.0;
        let mut hy_sz = 0usize; let mut hy_ba = 0.0;
        let mut cnt = 0;
        
        for img in &loaded {
            if let (Some(jp), Some(hy)) = (encode_with_config(&jpegli_cfg, img),
                                            encode_with_config(&hybrid_cfg, img)) {
                if let (Some(jp_b), Some(hy_b)) = (compute_butteraugli(img, &jp),
                                                    compute_butteraugli(img, &hy)) {
                    jp_sz += jp.len(); jp_ba += jp_b;
                    hy_sz += hy.len(); hy_ba += hy_b;
                    cnt += 1;
                }
            }
        }
        ...
        let p = if sz_d < -0.5 && ba_d < -0.5 { " ★" } else { "" };
        
        println!("{:>6.0} {:>12.0} {:>10.3} {:>12.0} {:>10.3} {:>+10.1}% {:>+10.1}%{}",
        ...
        for offset in quality_offsets {
            ...
            if sz_d < 0.0 && ba_d < 0.0 {
                ...
                if best.is_none() || score > best.unwrap().2 + best.unwrap().3 {
                    ...
                    println!("★ PARETO: coupling={:+.1}, Q{:.1} → {:.1}% smaller, {:.1}% better BA",
