...
fn main() {
    use zenjpeg::quant::aq::{autovec, simd};
    ...
    println!(
        "gamma: wide={:.6}, autovec={:.6}, diff={:.6}",
        gamma_wide,
        gamma_autovec,
        (gamma_wide - gamma_autovec).abs()
    );
    ...
    println!(
        "hf: wide={:.6}, autovec={:.6}, diff={:.6}",
        hf_wide,
        hf_autovec,
        (hf_wide - hf_autovec).abs()
    );
