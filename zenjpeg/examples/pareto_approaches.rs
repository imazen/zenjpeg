...
fn main() {
    ...
    println!("Goal: Same butteraugli as jpegli Q{}, smaller file\n", base_quality);
    ...
    println!("Test: hybrid Q{}+N with aq_lambda_scale={}\n", base_quality, negative_coupling);
    ...
    for boost in quality_boosts {
        ...
        if count > 0 {
            ...
            let pareto = if size_delta < 0.0 && ba_delta <= 0.5 { " ★" } else { "" };
            
            println!("{:>6.1} {:>10.0} {:>10.3} {:>10.0} {:>10.3} {:>+10.1}% {:>+10.1}%{}",
                     boost, jpegli_size_avg, jpegli_ba_avg, hybrid_size_avg, hybrid_ba_avg,
                     size_delta, ba_delta, pareto);
        }
    }
    
    println!("\n=== APPROACH D: Positive Coupling (preserve texture, reduce base quality) ===\n");
    println!("Goal: Same file size as jpegli Q{}, better quality\n", base_quality);
    ...
    println!("Baseline: jpegli Q{} (no hybrid)", base_quality);
    println!("Test: hybrid Q{}-N with positive aq_lambda_scale\n", base_quality);
    ...
    for coupling in positive_couplings {
        ...
        for reduction in &quality_reductions {
            ...
            if count > 0 {
                ...
                let pareto = if size_delta.abs() < 2.0 && ba_delta < 0.0 { " ★" } else { "" };
                
                println!("{:>6.1} {:>10.0} {:>10.3} {:>10.0} {:>10.3} {:>+10.1}% {:>+10.1}%{}",
                         reduction, jpegli_size_avg, jpegli_ba_avg, hybrid_size_avg, hybrid_ba_avg,
                         size_delta, ba_delta, pareto);
            }
        }
    }
