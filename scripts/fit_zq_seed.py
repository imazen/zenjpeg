#!/usr/bin/env python3
"""Fit the zenjpeg Zq seed head (see benchmarks/zq_seed_wave_2026-08-26.md).

Mirrors zenavif scripts/hyperparam/fit_q0_head.py (the directive exemplar) on
the 07-01 canonical zenjpeg_lossy picker set. Emits:
...
BASE = "/mnt/v/output/canonical-picker-2026-07-01-zensimA/zenjpeg_lossy"
...
def main():
    ...
    # G-Z2 sim: head vs anchor on validate curves
    def head_q0(c, t):
        tn=(t-65)/25; h80=max(t-80,0)/10
        fv=[featval(c,n) for n in chosen]
        if any(v is None for v in fv): return None
        x = [1.0,tn]+[max(t-k,0)/10 for k in (50,60,70,80,85)]+[(math.log(c["px"])-13)/3] \
            + fv + [v*tn for v in fv] + [v*h80 for v in fv]
        return float(np.dot(beta, x))
    encA=[]; encH=[]; convA=0; convH=0; n=0
    for k, c in va.items():
        for t in TARGETS:
            if invert(c, t) is None: continue
            n += 1
            qa,_ ,ca, ea = simulate(c, float(t), anchor_guess(t))
            q0 = head_q0(c, float(t))
            qh,_, ch, eh = simulate(c, float(t), q0 if q0 is not None else anchor_guess(t))
            encA.append(ea); encH.append(eh); convA += ca; convH += ch
    mA, mH = float(np.mean(encA)), float(np.mean(encH))
    imp = (mA-mH)/mA*100
    g2 = imp >= 10.0 and convH >= convA
    print(f"G-Z2 sim over {n} cells: anchor mean {mA:.2f} enc (conv {convA}) | head mean {mH:.2f} (conv {convH}) | improvement {imp:.1f}% -> {'PASS' if g2 else 'FAIL'}", file=sys.stderr)

    with open("benchmarks/zq_seed_fit_2026-08-26.tsv","w") as f:
        f.write("# zenjpeg zq seed fit 2026-08-26 — see zq_seed_wave_2026-08-26.md\n")
        f.write(f"# features\t{','.join(chosen)}\n")
        f.write(f"g_z1_val_p50\t{p50v:.4f}\ng_z1_val_p90\t{p90v:.4f}\n")
        f.write(f"sim_cells\t{n}\nsim_anchor_mean_encodes\t{mA:.4f}\nsim_head_mean_encodes\t{mH:.4f}\n")
        f.write(f"sim_anchor_converged\t{convA}\nsim_head_converged\t{convH}\nsim_improvement_pct\t{imp:.2f}\n")
        f.write(f"g_z2\t{'PASS' if g2 else 'FAIL'}\n")
        f.write("coef\t" + ",".join(f"{b:.10e}" for b in beta) + "\n")
    print(json.dumps({"chosen": chosen, "coefs": [float(b) for b in beta],
                      "g_z1": {"p50": p50v, "p90": p90v},
                      "g_z2": {"pass": g2, "impr_pct": imp, "anchor_mean": mA, "head_mean": mH,
                               "conv_anchor": convA, "conv_head": convH, "cells": n}}))
    return 0
